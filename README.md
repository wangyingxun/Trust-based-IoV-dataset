...
Over the past decade or so, the notion of trust has gained increasing attention of researchers from both academia and industry. Accordingly, the manuscript-at-hand presents an IoV simulator envisaged via Java. To ensure a realistic outlook, we have not only incorporated trustworthy vehicles but have also catered for malicious vehicles which dynamically switch between trustworthy and untrustworthy behaviors to execute trust-based attacks in order to evade detection within an IoV network.

The envisaged trust-based IoV dataset encompasses 96 vehicles (trustors and trustees) that have carried out a total of 1,048,576 interactions, i.e., both positive and negative, at different time instances. The dataset is in a tabular format and comprises 10 columns, i.e., Interaction Time, Trustor, Trustee, four trust parameters (Interaction Experience, Interaction Frequency, Interaction Timeliness, and Received Message Quality), Total Trust, and two labels (MaliciousVehicle and IsAttacking). The details of the same are delineated as follows:
